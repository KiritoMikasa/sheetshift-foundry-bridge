import { CHANNEL, isId, validRequest } from "./protocol.mjs";

// No socket relay or GM impersonation: only the window opened by this player can send checks.
export function createReceiver({ source, origin, nonce, execute, send, storage, storageKey, isOnline = () => true, now = Date.now }) {
  ...
  return async function receive(event) {
    ...
    if (!validRequest(request)) { reply("result", { id: request.id, error: "Ungültige Probe." }); return; }
